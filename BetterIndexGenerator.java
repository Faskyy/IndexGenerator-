...
public class BetterIndexGenerator {
   ...
   while (scanner.hasNextLine()){
      lineNum++;// incremenet line number
      st = new StringTokenizer(scanner.nextLine()); //assigned stringtokenizer to tokenize the line
      while(st.hasMoreTokens()) // has more tokens
         {
            List<Integer> lines; //declare a list
            word = st.nextToken(); //word equals next token
            lines = wordToListMap.getOrDefault(word, new ArrayList<>());
            //get value mapped to this key, passed as the parameter.if not get default value
            lines.add(lineNum); //add line num to line
            wordToListMap.put(word, lines);// put updates the list
         }
      }
...
//      // Display the TreeMap sorted by key
   ...
   public static void main(String[] args) throws FileNotFoundException {
     ...
     // Scanner scan = new Scanner(new FileInputStream("tale.txt"));
      ...
      // build the mapping from words to line numbers
      //exercise 2
